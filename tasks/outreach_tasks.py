        ...
        for seq in pending:
            ...
            try:
                ...
                new_count = await redis_client.eval(
                    ...
                    settings.daily_email_limit, settings.SECONDS_PER_DAY
                )
...
async def _send_step_email(db, brevo, seq, step, idempotency_key) -> bool:
    ...
    try:
        ...
        logger.error(f"Template rendering failed for {template_name}")
        return False
    ...
    async with DatabaseTransaction() as conn:
        await conn.execute(
            "INSERT INTO idempotency_keys (key, status, created_at, expires_at) VALUES ($1, 'processing', NOW(), NOW() + INTERVAL '7 days') ON CONFLICT (key) DO NOTHING",
            idempotency_key
        )
        ...
        await conn.execute(
            "INSERT INTO consent_log (prospect_id, consent_type, consent_text, source) VALUES ($1, 'legitimate_interest', 'Business contact for affiliate partnership inquiry', 'outreach_sequence') ON CONFLICT DO NOTHING",
            seq["prospect_id"]
        )
...
async def _advance_sequence(db, seq, steps):
...
async def _complete_sequence(db, sequence_id):
    await db.execute(
...
async def _stop_sequence(db, sequence_id, reason):
    await db.execute(
