...
Celery tasks for email extraction and verification
"""
...
def run_email_extraction(self):
    """
    Extract emails from prospect profiles using hybrid approach.
    
    Runs every 6 hours via Celery Beat.
    Uses HTTP+BeautifulSoup first, Playwright fallback.
    """
    loop = asyncio.get_event_loop()
    return loop.run_until_complete(_email_extraction_async())


async def _email_extraction_async() -> dict:
    """Async implementation of email extraction."""
    db = get_database()
    ...
    try:
        ...
        extraction_results = await extractor.extract_for_prospects(
            limit=30,
            only_missing=True
        )
        ...
        logger.warning("Hybrid email extractor not available, trying basic extraction")
        ...
        logger.info(f"Found {len(prospects)} prospects needing email extraction")
...
def run_email_verification(self):
    """
    Verify extracted emails using Clearout/Hunter.
    
    Runs every 6 hours (30 min after extraction) via Celery Beat.
    """
    loop = asyncio.get_event_loop()
    return loop.run_until_complete(_email_verification_async())


async def _email_verification_async() -> dict:
    """Async implementation of email verification."""
    ...
    # Skip if verification not configured
    if not settings.clearout_api_key and not settings.hunter_api_key:
        logger.warning("Email verification not configured, skipping")
    ...
    try:
        ...
        verification_results = await client.verify_batch(
            limit=100,
        ...
        logger.warning("Email verification module not available")
        
        # Fallback: just mark emails as unverified
        await db.execute(
            """
